...
import { localized, localizedDescription, t } from "@/lib/i18n";
...
export default async function ProductPage({ params }: Params) {
  ...
  const description = localizedDescription(product.description, product.descriptionFa, lang);
  const waUrl = whatsappProductUrl(displayName, product.price, product.currency);
  const msUrl = messengerProductUrl(displayName);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "start" }}
        ...
        <div>
          <div style={{ borderRadius: 16, overflow: "hidden", border: "1px solid var(--border-color)", background: "#f5f0eb", aspectRatio: "3/4", position: "relative" }}>
            {product.images[0] ? (
              <Image
                src={product.images[0]} alt={displayName}
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
              {product.images.slice(1).map((img, i) => (
                <div key={i} style={{ width: 72, height: 72, borderRadius: 8, overflow: "hidden", border: "1px solid var(--border-color)", background: "#f5f0eb", position: "relative" }}>
                  <Image src={img} alt={`${displayName} ${i + 2}`} fill style={{ objectFit: "contain" }} sizes="72px" />
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            <span style={{
              ...
              {product.inStock ? t("in_stock", lang) : t("out_of_stock", lang)}
            </span>
          </div>

          {description && (
            <p style={{ color: "var(--muted)", lineHeight: 1.8, fontSize: 14, marginBottom: 24, whiteSpace: "pre-line" }}>
              {description}
            </p>
          )}
          ...
          <AddToCartButton product={{
            id: product.id, name: displayName, price: product.price,
          ...
          {/* Order directly */}
          <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 12, fontWeight: 500 }}>{t("or_order_directly", lang)}</p>
