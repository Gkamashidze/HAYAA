  ...
  if (items.length === 0) {
    return (
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "5rem 1.5rem", textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: "1.75rem", marginBottom: 12 }}>{t("cart_empty")}</h1>
        <p style={{ color: "var(--muted)", marginBottom: 24, fontSize: 14 }}>{t("cart_empty_sub")}</p>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: "2rem", fontWeight: 600 }}>{t("cart_title")}</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "2rem", alignItems: "start" }} className="cart-layout">
            ...
            <div key={item.id} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
                  ...
                  <span style={{ marginInlineStart: "auto", fontWeight: 600, fontSize: 14 }}>
                    {(item.price * item.quantity).toFixed(2)} {item.currency}
                  </span>
                  <button onClick={() => removeItem(item.id)}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#cc4444", fontSize: 12 }}>{t("remove")}</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ background: "white", borderRadius: 12, border: "1px solid var(--border-color)", padding: "1.5rem", minWidth: 260, position: "sticky", top: 90 }}>
          <h2 style={{ fontFamily: "Georgia, serif", fontSize: "1.2rem", fontWeight: 600, marginBottom: 16 }}>{t("order_summary")}</h2>
